...
public class DoubleTournamentPanel extends TournamentPanel {
    ...
    public void paintComponent(Graphics g){
        ...
        //drawing the polka dotted background
        drawPolkaDots(g);
    ...
    private void drawRound(Graphics g, int workingX, int workingWinY, int workingLoseY, int verticalWinSpace, int verticalLoseSpace, int roundNum, ArrayList<MatchBox[]> boxes){
        ...
        for (int matchNum = 1; matchNum <= tournament.getNumberOfMatchesInRound(roundNum); matchNum++){ //iterates through each match
            ...
            g.setColor(new Color(255, 187, 224));
        ...
        g.setColor(new Color(213, 151, 186)); //sets color of lines to dark pink
    ...
    private void drawLines(Graphics g, ArrayList<MatchBox[]> boxes){
        g.setColor(new Color(213, 151, 186));
    ...
    public void drawPolkaDots(Graphics g) {
        ...
        //drawing the polka dots by rows
        while (y < maxY) {
            while (x < maxX) {
                g.fillOval(x, y, HORIZONTAL_SPACE, HORIZONTAL_SPACE);
                x += HORIZONTAL_SPACE * 2;
            }
            if (y % (HORIZONTAL_SPACE * 5 / 2) == 0) {
                //alternates the dots' starting location
                x = 0;
            } else {
                x = -HORIZONTAL_SPACE;
            }
            y += HORIZONTAL_SPACE * 5 / 4;
        }
    }
